...
def _normalize_usage(raw_usage, total_cost_usd=None, cache_read_field="cache_read_input_tokens", cache_creation_field="cache_creation_input_tokens", allow_raw_total_cost=True):
    ...
    normalized = {
        ...
        "total_cost_usd": total_cost_usd if total_cost_usd is not None else (raw_usage.get("total_cost_usd") if allow_raw_total_cost else None),
    }
        ...
        if agent == "codex":
            if obj.get("type") == "turn.completed" and isinstance(obj.get("usage"), dict):
                usage = _normalize_usage(
                    obj["usage"],
                    cache_read_field="cached_input_tokens",
                    cache_creation_field="cache_write_input_tokens",
                    allow_raw_total_cost=False,
                )
        elif agent == "claude":

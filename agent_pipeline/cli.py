...
def build_parser():
    ...
    init = sub.add_parser("init", help="scaffold editable Catenna state")
    init.add_argument("--force", action="store_true", help="overwrite orchestrator config with defaults")
    init.add_argument("--codex-model", help="set agents.codex.model in the written config (pricing.codex still needs its own rates for real-cost accounting)")
    init.set_defaults(func=lambda a: controller.pipeline_init(force=a.force, codex_model=a.codex_model))
    ...
    run.add_argument("--background", "--bg", action="store_true", help="launch and return immediately")
    ...
    verify.add_argument("--background", "--bg", action="store_true", help="launch and return immediately")
